...
// Ported from API for perceptually accurate color blending
// ============================================================================
...
function xyzToLab(xyz) {
  const fx = labF(xyz.X / D65_REFERENCE.Xn);
  const fy = labF(xyz.Y / D65_REFERENCE.Yn);
  const fz = labF(xyz.Z / D65_REFERENCE.Zn);
...
function labToXYZ(lab) {
  ...
  return {
    X: D65_REFERENCE.Xn * labFInv(fx),
    Y: D65_REFERENCE.Yn * labFInv(fy),
    Z: D65_REFERENCE.Zn * labFInv(fz)
  };
}
...
// ============================================================================
// End LAB Conversion Functions
// ============================================================================

/**
 * Calculate blended color from parts using LAB color space weighted averaging
 * This matches the API's calculation method for consistent color results
 *
...
export function calculateBlendedColor(parts) {
  ...
  // Perform weighted average in LAB space (perceptually accurate)
  let weightedL = 0;
  let weightedA = 0;
  let weightedB = 0;
  ...
  parts.forEach((count, colorIndex) => {
    ...
    // Use pre-calculated LAB values from palette
  ...
  return `#${toHex(blendRgb.R)}${toHex(blendRgb.G)}${toHex(blendRgb.B)}`;
}
