-- Database schema for installation translations (UUID version)
-- This creates the installation_i18n table to store translated content
-- UPDATED: Uses UUID for installation_id to match existing installations table

-- Drop table if it exists (in case of previous failed attempts)
DROP TABLE IF EXISTS installation_i18n;

-- Create the translation table with UUID support
CREATE TABLE installation_i18n (
    ...
    -- Foreign key to installations table (UUID reference)
    FOREIGN KEY (installation_id) REFERENCES installations(id) ON DELETE CASCADE
);

-- Create indexes for performance
CREATE INDEX idx_installation_i18n_lookup ON installation_i18n(installation_id, lang);
CREATE INDEX idx_installation_i18n_lang ON installation_i18n(lang);
CREATE INDEX idx_installation_i18n_slug ON installation_i18n(slug);
...
COMMENT ON COLUMN installation_i18n.installation_id IS 'Foreign key reference to installations table (UUID)';
...
WHERE table_name = 'installation_i18n' 
